...
resource "aws_instance" "web" {
  ...
  user_data = <<-EOF
    ...
    docker run -d --name wordpress_mysql --network net dkunderenko/task9:wordpress-mysql
    docker run -d --name wordpress_nginx --network net -p 80:80 dkunderenko/task9:wordpress-nginx
  EOF
